...
import { Meter, EventStrip, HeadroomCard, SettingsSheet } from "@/components/tuner";
...
export default function TunePage() {
  ...
  const onOnset = useCallback((window: Float32Array, sampleRate: number) => {
    ...
    // An unclear strike — one that did not agree with itself across
    // sub-windows — is dropped entirely. Better to leave the display where it
    // is than to move it on a reading we do not trust (RULES B4).
    if (!onset) return;
  ...
  const { cents, trail, pendingTaps, centsPerTap } = snapshot;
